...
public class UserProcessor {
     ...
     */
    public List<FailedEmails> process(final StaxStreamProcessor processor, List<City> cities,  int chunkSize) throws XMLStreamException, JAXBException {
        ...
        Map<String, City> cityMap = cities.stream().collect(Collectors.toMap(City::getRef, v->v));


        while (processor.doUntil(XMLEvent.START_ELEMENT, "User")) {
            String cityRef = processor.getAttribute("city");
            ru.javaops.masterjava.xml.schema.User xmlUser = unmarshaller.unmarshal(processor.getReader(), ru.javaops.masterjava.xml.schema.User.class);
            if (cityRef== null|| cityRef.isEmpty()||!cityMap.containsKey(cityRef)){
                failed.add(new FailedEmails(xmlUser.getEmail(), "Not correct city ref: " + cityRef));
                continue;
            }
